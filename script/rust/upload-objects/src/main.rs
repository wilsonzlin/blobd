...
async fn worker(worker_state: Arc<WorkerState>) -> Result<(), Box<dyn Error>> {
...
async fn main() -> Result<(), Box<dyn Error>> {
  ...
  for _ in 0..args.concurrency {
    workers.push(worker(worker_state.clone()));
  };
  ...
  println!("Effective processing rate: {} seconds", args.count as f64 / elapsed);
