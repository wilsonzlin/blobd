...
impl Flusher {
  pub async fn start_loop(&mut self) {
    while let Some(pending) = self.receiver.recv().await {
      self.dev.write_at(self.offset, &pending.buf).await;

      // We can immediately ACK clients because they're now durable on disk, even if only in journal.
      ...
      self.dev.write_at(self.offset, &BLANK_STATE).await;
...
impl Journal {
  pub fn new(dev: Arc<dyn IDevice>, dev_offset: u64, cap: usize) -> Arc<Self> {
...
impl IJournal for Journal {
  ...
  fn commit_transaction(&self, txn: Transaction) -> Option<SignalFuture<()>> {
    ...
    let (signal, signal_ctl) = SignalFuture::<()>::new();
    {
      ...
      let flusher_is_idle_and_journal_meets_min_write = self.in_flush_queue.load(Relaxed) > 0 && pending.buf.len() >= 512;
