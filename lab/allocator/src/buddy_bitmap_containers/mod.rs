...
impl Allocator {
  ...
  pub fn initial_bitmap_container_page_count(
    ...
    let usable_dev_size = floor_pow2(dev_size, lpage_size_pow2);
    ...
    for page_size_pow2 in spage_size_pow2..=lpage_size_pow2 {
      let page_count = div_pow2(usable_dev_size, page_size_pow2);
  ...
  pub async fn format_device(
    ...
    // For calculation simplicity and correctness, we only use up to the last full lpage on the device.
    let usable_dev_size = floor_pow2(dev_size, lpage_size_pow2);
