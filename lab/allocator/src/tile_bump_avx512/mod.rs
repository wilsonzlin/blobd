...
use super::FreeList;
use crate::ndtreearray::NDTreeArray;
use crate::tile::TILE_SIZE;
use itertools::Itertools;
use off64::usz;
use off64::Off64;
use seekable_async_file::SeekableAsyncFile;
use std::arch::x86_64::_mm512_cmpge_epu32_mask;
use std::arch::x86_64::_mm512_cmpneq_epu64_mask;
use std::arch::x86_64::_mm512_loadu_epi64;
use std::arch::x86_64::_mm512_loadu_epi8;
use std::arch::x86_64::_mm512_mask_blend_epi8;
use std::arch::x86_64::_mm512_mask_compress_epi8;
use std::arch::x86_64::_mm512_mask_compressstoreu_epi8;
use std::arch::x86_64::_mm512_reduce_max_epu32;
...
// The code in this file was ported from the C version (originally called Turbostore). See src/freelist.c and src/vec.c from the tree at commit ffb637ae4e4e91602ec04cf2fb2b50aafa116876.

unsafe fn find_indices_of_nonzero_bits_64(bits: u64) -> [u8; 64] {
  const INDICES_RAW: [i8; 64] = [
...
unsafe fn find_set_bit_sequences_in_bitmap_64(bitmap: u64) -> impl Iterator<Item = (u64, u64)> {
  ...
  let range_boundaries: i8x64 =
    _mm512_mask_compress_epi8(fill, range_boundaries_mask, indices).into();
  ...
  range_boundaries
    .to_array()
    .into_iter()
    .map(|idx| u64::try_from(idx).unwrap())
    .tuples()
}
...
impl Avx512FreeList {
  ...
  unsafe fn fast_allocate_one_tile(&mut self) -> u32 {
    ...
    self.propagate_tile_bitmap_change(
      self.tile_bitmaps.get([i1, i2, i3]) & !(1u64 << i4),
      i1,
      i2,
      i3,
    );
...
impl FreeList for Avx512FreeList {
  fn load_from_device(&mut self, device: SeekableAsyncFile, tile_count: u64) {
    ...
    for tile_no in 0..tile_count {
      ...
      let m6 = u32::try_from(itmp % 16).unwrap();
      itmp /= 16;
      let m5 = itmp % 16;
      itmp /= 16;
      let m4 = itmp % 16;
    ...
    eprintln!(
      "{} free tiles, {} microtiles",
      free_tile_count, microtile_count
    );
    ...
    unsafe {
      ...
      for i1 in 0u64..64 {
        for i2 in 0u64..64 {
          for i3 in (0u64..64).step_by(8) {
            let m: u64 = _mm512_cmpneq_epu64_mask(
              _mm512_loadu_epi64(
      ...
      for i1 in 0u64..64 {
        for i2 in (0u64..64).step_by(8) {
          let m: u64 = _mm512_cmpneq_epu64_mask(
            _mm512_loadu_epi64(self.tile_bitmaps.get_ref([i1, i2]) as *const u64 as *const i64),
            zeroes,
          )
          .into();
      ...
      for i1 in (0u64..64).step_by(8) {
        let m: u64 = _mm512_cmpneq_epu64_mask(
          _mm512_loadu_epi64(self.tile_bitmaps.get_ref([i1]) as *const u64 as *const i64),
          zeroes,
        )
        .into();
      ...
      for i1 in 0u32..16 {
        for i2 in 0u32..16 {
          for i3 in 0u32..16 {
            for i4 in 0u32..16 {
              for i5 in 0u32..16 {
                let m = _mm512_reduce_max_epu32(
                  self.microtile_free_maps.get([i1, i2, i3, i4, i5]).into(),
                );
                self.microtile_free_maps.get_mut([i1, i2, i3, i4])[usz!(i5)] = (m & !15) | i5;
              }
              let m =
                _mm512_reduce_max_epu32(self.microtile_free_maps.get([i1, i2, i3, i4]).into());
  ...
  fn consume_microtile_space(&mut self, bytes_needed: u32) -> u64 {
    unsafe {
      ...
      let (microtile_addr, cur_free, i1, i2, i3, i4, i5, i6) = if p1 == 32 {
        ...
        eprintln!(
          "No existing microtile available, so converting tile {}",
          microtile_addr
        );
        ...
        let m5 =
          _mm512_cmpge_epu32_mask(self.microtile_free_maps.get([i1, i2, i3, i4]).into(), y512);
        ...
        let m6 = _mm512_cmpge_epu32_mask(
          self.microtile_free_maps.get([i1, i2, i3, i4, i5]).into(),
          y512,
        );
        ...
        eprintln!(
          "Found microtile {} with {} free space",
          microtile_addr, cur_free
        );
        (microtile_addr, cur_free, i1, i2, i3, i4, i5, i6)
      };
      ...
      eprintln!(
        "Microtile {} now has {} free space",
        microtile_addr, new_free
      );
      self.microtile_free_maps.get_mut([i1, i2, i3, i4, i5])[usz!(i6)] = (new_free << 8) | i6;
      self.microtile_free_maps.get_mut([i1, i2, i3, i4])[usz!(i5)] =
        (_mm512_reduce_max_epu32(self.microtile_free_maps.get([i1, i2, i3, i4, i5]).into()) & !15)
          | i5;
      self.microtile_free_maps.get_mut([i1, i2, i3])[usz!(i4)] =
        (_mm512_reduce_max_epu32(self.microtile_free_maps.get([i1, i2, i3, i4]).into()) & !15) | i4;
      self.microtile_free_maps.get_mut([i1, i2])[usz!(i3)] =
        (_mm512_reduce_max_epu32(self.microtile_free_maps.get([i1, i2, i3]).into()) & !15) | i3;
      self.microtile_free_maps.get_mut([i1])[usz!(i2)] =
        (_mm512_reduce_max_epu32(self.microtile_free_maps.get([i1, i2]).into()) & !15) | i2;
      self.microtile_free_maps.root_mut()[usz!(i1)] =
        (_mm512_reduce_max_epu32(self.microtile_free_maps.get([i1]).into()) & !15) | i1;
      ...
      'outer: for i1 in i1_candidates {
        ...
        for i2 in i2_candidates {
          ...
          for i3 in i3_candidates {
            ...
            let result =
              find_free_tiles_in_region(bitmap, min(tiles_remaining.try_into().unwrap(), 64));
