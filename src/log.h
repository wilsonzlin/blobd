...
#define LOGGER(subsystem) \
  static void log(log_level_t lvl, char const* format, ...) { \
    va_list args; \
...
#define CORRUPT(fmt, ...) \
  log(CRIT, "Corruption detected: " fmt "\nThis may be due to bit flip/rot, external tampering, or imminent hardware failure. It may not be safe to continue using the device, so exiting now for safety.", ##__VA_ARGS__); \
  exit(EXIT_CORRUPT)
